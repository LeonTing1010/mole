...
pub fn generate(
    ...
    let dns = json!({
        "servers": [
            ...
            { "tag": "remote-alt", "type": "https", "server": "8.8.8.8", "detour": "proxy" },
    ...
    // --- Inbounds ---
    // Collect proxy server IPs to exclude from VPN routing
    // This prevents routing loop: traffic to proxy server should go direct, not through VPN
    let mut exclude_ips: Vec<String> = Vec::new();
    for (_, node) in nodes {
        let addr = node.server_addr();
        if let Some(ip) = addr.split(':').next() {
            // Only add if it's a valid IP address (not a domain)
            if let Ok(parsed) = ip.parse::<std::net::IpAddr>() {
                // Convert to CIDR format: IPv4 uses /32, IPv6 uses /128
                let cidr = match parsed {
                    std::net::IpAddr::V4(_) => format!("{}/32", ip),
                    std::net::IpAddr::V6(_) => format!("{}/128", ip),
                };
                exclude_ips.push(cidr);
            }
        }
    }
    ...
    let mut tun_inbound = json!({
        ...
        "mtu": 1500, "auto_route": false, "strict_route": false, "stack": "mixed"
    });

    // Add route_exclude_address to prevent routing loop
    if !exclude_ips.is_empty() {
        tun_inbound["route_exclude_address"] = json!(exclude_ips);
    }

    let mut inbound_list = vec![tun_inbound];
    ...
    // Add direct rules for local/private networks to avoid TUN overhead
