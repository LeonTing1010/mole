...
pub fn run_strategy(s: &store::Store, strat: &str, bypass_cn: bool, is_daemon: bool, share: bool, ip_info: Option<IpInfo>) {
    ...
    if !is_daemon {
        status::start_live_monitor(thread_stop.clone(), ip_info);
    }
    if share && !is_daemon {
        print_share_info();
    }
...
pub fn run_single(s: &store::Store, bypass_cn: bool, is_daemon: bool, share: bool, ip_info: Option<IpInfo>) {
    ...
    loop {
        ...
        // 获取服务器地址用于显示
        let server_addr = parsed.server_addr();

        // 尝试获取服务器 IP 的地理位置
        let server_ip = server_addr.split(':').next().unwrap_or(&server_addr);
        let display_ip_info = if server_ip.parse::<std::net::IpAddr>().is_ok() {
            // 如果是 IP 地址，尝试查询地理位置
            match status::fetch_ip_geolocation(server_ip) {
                Ok(info) => info,
                Err(_) => IpInfo {
        ...
        let _keepalive = runner::start_keepalive(thread_stop.clone());
        if !is_daemon {
            status::start_live_monitor(thread_stop.clone(), Some(display_ip_info.clone()));
        }
