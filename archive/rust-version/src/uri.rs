...
/// Extract host from a parsed URL, stripping IPv6 brackets.
/// `Url::host_str()` returns `[::1]` for IPv6; sing-box expects `::1`.
fn extract_host(parsed: &Url) -> Result<String, String> {
    let h = parsed.host_str().ok_or("missing host")?;
...
mod tests {
    ...
    #[test]
    fn parse_hy2_ipv6_no_brackets() {
        let uri = "hy2://bd2f8047c892166b@[2001:19f0:0:3204:5400:6ff:fe12:59fd]:443?insecure=1&sni=bing.com#ipv6node";
        let node = ProxyNode::parse(uri).unwrap();
        let out = node.to_outbound("proxy");
        // sing-box expects raw IPv6 without brackets
        assert_eq!(out["server"], "2001:19f0:0:3204:5400:6ff:fe12:59fd");
    }
